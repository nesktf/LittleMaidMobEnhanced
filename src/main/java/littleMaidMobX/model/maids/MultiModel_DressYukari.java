...
*/
	...
	public void setLivingAnimationsLM(IModelCaps entityCaps, float f, float f1, float f2) {
		...
		IdOffset = entityId;
		pink.setVisible(ModelCapsHelper.getCapsValueBoolean(this, entityCaps, caps_isLookSuger));
	}	@Override
	public void setRotationAnglesLM(float f, float f1, float f2, float f3, float f4, float f5, IModelCaps entityCaps) {
		...
		Object ridingEntity = Helper.getRidingEntity(entity);
		int ticksExisted = Helper.getEntityTicksExisted(entity);
		if (ModelCapsHelper.getCapsValueBoolean(this, entityCaps, caps_isRiding)
				&& ridingEntity != null) t = (float)ticksExisted;
		else t = f2;		Ahoge.rotateAngleY = Helper.cos(t * 0.2F + Helper.cos(t * 0.05F + IdOffset) * 1.0F) * 0.2F;
	...
	public float getWidth(IModelCaps entityCaps) {
		...
		float v[][]={
				...
				specialModel=new ModelRenderer[4];
//				loadModel[0] = new ModchuModel_ModelRenderer(this);
				for(int i=0;i<specialModel.length;i++)
					specialModel[i] = new ModelRenderer(this);				for(int i=0;i<v.length;i++){
